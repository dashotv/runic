...
import { PopularMovie, Release, Response, Setting, runicClient } from '.';
...
export interface ReleasesMoviesResponse extends Response {
  result: PopularMovie[];
}
export const ReleasesMovies = async () => {
  const response = await runicClient.get(`/releases/popular_movies?`);
  ...
  return response.data as ReleasesMoviesResponse;
};
